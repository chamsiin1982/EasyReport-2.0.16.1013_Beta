...
public class TableReportService implements ITableReportService {
    ...
    @Override
    public ReportTable getReportTable(int id, Map<String, Object> formParams,Map<String, Object> appendParams) {
        Report report = this.reportService.getById(id);
        return this.getReportTable(report, formParams,appendParams);
    }

    @Override
    public ReportTable getReportTable(String uid, Map<String, Object> formParams,Map<String, Object> appendParams) {
        Report report = this.reportService.getByUid(uid);
        return this.getReportTable(report, formParams,appendParams);
    }

    @Override
    public ReportTable getReportTable(Report report, Map<String, Object> formParams,Map<String, Object> appendParams) {
        ReportDataSource reportDs = this.reportService.getReportDataSource(report.getDsId());
        return ReportGenerator.generate(reportDs, this.createReportParameter(report, formParams,appendParams));
    }
    ...
    @Override
    public ReportDataSet getReportDataSet(Report report, Map<String, Object> parameters,Map<String, Object> appendParams) {
        ReportDataSource reportDs = this.reportService.getReportDataSource(report.getDsId());
        return ReportGenerator.getDataSet(reportDs, this.createReportParameter(report, parameters,appendParams));
    }

    private ReportParameter createReportParameter(Report report, Map<String, Object> formParams) {        
        return this.createReportParameter(report,formParams,null);
    }
    /***
     * 根据报表信息 加载报表执行SQL
     * @param report
     ...
     * @return
     */
    private ReportParameter createReportParameter(Report report, Map<String, Object> formParams,Map<String, Object> appendParams) {
        String sqlText = new ReportSqlTemplate(report.getSqlText(), formParams).execute();
        
        try {
			sqlText=sqlTextAppend( sqlText,appendParams);
		} catch (Exception e) {
			e.printStackTrace();
			//;todo 抛出异常
		}
    		...
    		for(Object param: appendParams.values()){
    			...
    			if("and".equals(andor))
    			{
    				if(dataType.equals("string")||dataType.equals("date")) {
        				sql=JsqlParserWhereConditionHelper.addAndCondition(sql, columnName, operator, paramValue);
    				}else if(dataType.equals("integer")){
        				sql=JsqlParserWhereConditionHelper.addAndCondition(sql, columnName, operator,Long.parseLong(paramValue) );
    				}
    			}else if("or".equals(andor)){
    				if(dataType.equals("string")||dataType.equals("date")) {
        				sql=JsqlParserWhereConditionHelper.addOrCondition(sql, columnName, operator, paramValue);
    				}else if(dataType.equals("integer")){
        				sql=JsqlParserWhereConditionHelper.addOrCondition(sql, columnName, operator,Long.parseLong(paramValue) );
    				}
    			}
    		}
    	}
    ...
    public Map<String, Object> getFormParameters(Map<?, ?> httpReqParamMap, int dataRange) {
        ...
        this.setQueryParams(formParams, httpReqParamMap,queryParamsMap);
        return formParams;
    }
    
    public Map<String, Object> getQueryAppendParameters(Map<?, ?> httpReqParamMap, int dataRange) {
        Map<String, Object> formParams = new HashMap<>();
        Map<String, Object> queryParamsMap= new HashMap<>();
        //this.setBuildInParams(formParams, httpReqParamMap, 7);
        this.setQueryParams(formParams, httpReqParamMap,queryParamsMap);
     ...
     */
    private void setQueryParams(Map<String, Object> formParams, Map<?, ?> httpReqParamMap,Map<String,Object> queryParamsMap) {
